...
*/
	...
	for _, i := range s{
		...
		link := strings.Index(i, ")")
		left := i[:link]
		right := i[link + 1:]


		m[right] = append(m[right], left)
		findHowManyOrbits(left, s)
		m[right] = append(m[right], aux...)
		aux = nil
	}
	...
	fmt.Println("Find shortest path between:", "YOU", "and", "SAN")
...
func findHowManyOrbits(left string, s[] string) {
	...
	for _, i := range s{
		...
		if right == left {
			aux = append(aux, left_c)
			findHowManyOrbits(left_c, s)
		}
